...
- v1.1:
  - Remove the `reportError` method from `LambdaResponseWriter` and instead make the `handle` method of `StreamingLambdaHandler` throwing.
  - Remove the `addBackgroundTask` method from `LambdaContext` and introduce the `LambdaWithBackgroundTaskHandler` protocol as a solution
...
// TODO: With the addition of `LambdaWithBackgroundTaskHandler`, this paragraph does not hold much meaning anymore. Confirm whether to remove this paragraph entirely.
...
finishing functions from being called multiple times.
// TODO: Do we want to mention that we will automatically call `finish()` for the user if they have not called it?

### LambdaContext

`LambdaContext` will be largely unchanged, but the `eventLoop` property will be removed.
// TODO: Do we want to explain why `eventLoop` is being removed?

The `allocator` property of type `ByteBufferAllocator` will also be removed because (1), we generally want to reduce the
number of SwiftNIO types exposed in the API, and (2), `ByteBufferAllocator` does not optimize the allocation strategies.
The common pattern observed across many libraries is to re-use existing `ByteBuffer`s as much as possible. This is also
...
/// TODO: Check that we should mark this with `Sendable` given that `addBackgroundTask(_:)` has been removed
...
The HTTP response is started lazily before the first call to `LambdaResponseWriter.write()`. Bytes written to the writer
are streamed continually. A call to `LambdaResponseWriter.finish()` closes off the HTTP response.
...
>: StreamingLambdaHandler where Handler.Output == Output, Handler.Event == Event {
    ...
    public mutating func handle(
        ...
        responseWriter: consuming some LambdaResponseWriter, /// TODO: Confirm if we should correct this
        context: LambdaContext
    ) async throws // TODO: "-> LambdaResponse" incorrect initially. Confirm if we should correct the signature to not return LambdaHandler
}
```
...
Note that the `handle` method of the `LambdaHandler` protocol _does_ however return a generic `Output` object instead of
writing the response to a `LambdaResponseWriter`. The reason for this is that `LambdaHandler` is designed to be used
alongside `LambdaCodableAdapter` for the common JSON-in, JSON-out use-case, where the response cannot be streamed (since
each partial output has to be valid JSON for the output encoding that `LambdaCodableAdapter` performs).

For such use-cases requiring JSON abstraction as well as the ability to run background tasks after returning the
response, a new handler protocol, `LambdaWithBackgroundTaskHandler`, should be used. This protocol differs from
`LambdaHandler` in two ways:

1. The `handle` function has an additional argument accepting an object conforming to a new protocol `ResponseSender`.
   The `ResponseSender` protocol specifies a `send` function (instead of the `write` and `finish` semantics of
   `LambdaResponseWriter`).
2. The `handle` function does not return.

```swift
public struct ResponseSender<Output>: ~Copyable {
...
public protocol LambdaWithBackgroundTaskHandler {
    /// Generic input type
    /// The body of the request sent to Lambda will be decoded into this type for the handler to consume
    associatedtype Event
    /// Generic output type
