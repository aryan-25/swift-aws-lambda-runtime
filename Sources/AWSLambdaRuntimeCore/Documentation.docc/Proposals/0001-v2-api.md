...
- v1.1:
  - Remove the `reportError(_:)` method from `LambdaResponseWriter` and instead make the `handle(...)` method of
...
It is important to note that `LambdaResponseWriter` will be non-copyable, with the `finish()` and `writeAndFinish(_:)`
functions having the `consuming` ownership keyword. This is so that the compiler can enforce the restriction of not
being able to call `write(_:)` after any of the finishing functions have been called. The compiler can also prevent the
finishing functions from being called multiple times. If the user does not call `finish()`, the library will
automatically finish the stream in the writers `deinit`.
...
public protocol StreamingLambdaHandler: ~Copyable {
    ...
    ///   - responseWriter: A ``LambdaResponseWriter`` to write the invocation's response to.
    ...
    ///     ``LambdaResponseWriter.write(_:)``.
    ///   - If the error is thrown after call(s) to ``LambdaResponseWriter.write(_:)`` but before
    ///     a call to ``LambdaResponseWriter.finish()``, the response stream will be closed and trailing
    ///     headers will be sent.
    ///   - If ``LambdaResponseWriter.finish()`` has already been called before the error is thrown, the
    ///     error will be logged.
    mutating func handle(_ event: ByteBuffer, responseWriter: consuming some LambdaResponseWriter, context: LambdaContext) async throws
}
```
...
Through the `LambdaResponseWriter`, which is passed as an argument in the `handle` function, the **response can be
streamed** by calling the `write(_:)` function of the `LambdaResponseWriter` with partial data repeatedly before finally
closing the response stream by calling `finish()`. Users can also choose to return the entire output and not stream the
response by calling `writeAndFinish(_:)`.
...
Note that the `handle` function does not receive a `LambdaResponseWriter` as an argument. Response streaming is not
viable for `LambdaHandler` because the output has to be encoded prior to it being sent, e.g. it is not possible to
...
`LambdaOutputWriter` that is passed in as an argument, meaning that the `handle` function is then free to implement any
background work after the result has been sent to the AWS Lambda control plane.

`LambdaOutputWriter` has different semantics to the `LambdaResponseWriter`. Where the `write(_:)` function of
`LambdaResponseWriter` means writing into a response stream, the `write(_:)` function of `LambdaOutputWriter` simply
serves as a mechanism to return the output without explicitly returning from the `handle` function.

```swift
public protocol LambdaOutputWriter<Output>: ~Copyable {
    associatedtype Output
...
public protocol LambdaWithBackgroundProcessingHandler {
    ...
    associatedtype Event
    /// Generic output type
    /// This is the type that the handle() function will send through the ``LambdaOutputWriter``.
    associatedtype Output
    ...
    func handle(
        _ event: Event,
        outputWriter: consuming some LambdaOutputWriter<Output>,
        context: LambdaContext
    ) async throws
}
```
...
1. `LambdaHandlerAdapter` wraps `LambdaHandler` with `LambdaWithBackgroundProcessingHandler`. This is achieved by taking
   the generic `Output` object returned from the `handle` function of `LambdaHandler` and passing it to the `write(_:)`
   function of the `LambdaOutputWriter`.

2. `LambdaCodableAdapter` wraps `LambdaWithBackgroundProcessingHandler` with `StreamingLambdaHandler`. This is achieved
   by wrapping the `LambdaOutputWriter` with the `LambdaResponseWriter` provided by `StreamingLambdaHandler`. A call to
   the `write(_:)` function of `LambdaOutputWriter` is translated into a call to the `writeAndFinish(_:)` function of
   `LambdaResponseWriter`.

Both `LambdaHandlerAdapter` and `LambdaCodableAdapter` are described in greater detail in the **Codable Support**
section.

To summarize, `LambdaHandler` is first wrapped with `LambdaWithBackgroundHandler` (through `LambdaHandlerAdapter`) and
then with `StreamingLambdaHandler` (through `LambdaCodableAdapter`). `LambdaWithBackgroundHandler` just requires
...
>: LambdaWithBackgroundProcessingHandler where Handler.Event == Event, Handler.Output == Output {
    ...
    public func handle(_ event: Event, outputWriter: consuming some LambdaOutputWriter<Output>, context: LambdaContext) async throws
}
```
...
2.  Wrapping the `LambdaResponseWriter` with a concrete `LambdaOutputWriter` such that calls to `LambdaOutputWriter`s
    `write(_:)` are mapped to `LambdaResponseWriter`s `writeAndFinish(_:)`.
    - Note that the argument to `LambdaOutputWriter`s `write(_:)` is a generic `Output` object whereas
      `LambdaResponseWriter`s `writeAndFinish(_:)` requires a `ByteBuffer`.
    - Therefore, the concrete implementation of `LambdaOutputWriter` also accepts an encoder. Its `write(_:)` function
      first encodes the generic `Output` object and then passes it to the underlying `LambdaResponseWriter`. // TODO:
      confirm whether this is the approach we intend to go with.
3.  Passing the generic `Event` instance, the concrete `LambdaOutputWriter`, as well as the `LambdaContext` to the
...
>: StreamingLambdaHandler where Handler.Output == Output, Handler.Event == Event {
    ...
    /// 2. Create a concrete `LambdaOutputWriter` that maps calls to `write(_:)` with the `responseWriter`s `writeAndFinish(_:)`
    /// 2. Call the underlying `self.handler.handle()` method with the decoded event data, the concrete `LambdaOutputWriter`,
...
work could be executed within the `handle(...)` function in a structured concurrency manner after the call to
`LambdaResponseWriter.finish()`. For handlers conforming to the `LambdaHandler` protocol, this solution was considered:
...
The most common use-case for this library will be to implement simple Lambda functions that do not have requirements for
response streaming, nor to perform any background work after returning the output. To keep things easy for the common
use-case, and with Swift's principle of progressive disclosure of complexity in mind, we settled on three handler
protocols:
...
1. `LambdaHandler`: Most common use-case. JSON-in, JSON-out. Does not support background work execution. An intuitive
   ...
   to the concept of sending their response through a `LambdaOutputWriter`. `LambdaHandler` can be very cleanly
   implemented, especially with `ClosureHandler`.
2. `LambdaWithBackgroundProcessingHandler`: If users wish to augment their `LambdaHandler` with the ability to run
   ...
   2. Add an additional `outputWriter: some LambdaOutputWriter<Output>` argument to the `handle` function.
...
3. `StreamingLambdaHandler`: This is the base handler protocol which is intended to be used directly only for advanced
   use-cases. Users are provided the invocation event as a `ByteBuffer` and a `LambdaResponseWriter` where the computed
   result (as `ByteBuffer`) can either be streamed or sent all at once. After closing the `LambdaResponseWriter` (with
   `finish()` or `writeAndFinish()`), any background work can be implemented.
