...
- v1.1:
  - Remove the `reportError` method from `LambdaResponseWriter` and instead make the `handle` method of
    `StreamingLambdaHandler` throwing.
  - Remove the `addBackgroundTask` method from `LambdaContext` and introduce the `LambdaWithBackgroundProcessingHandler`
...
// TODO: With the addition of `LambdaWithBackgroundProcessingHandler`, this paragraph does not hold much meaning
anymore. Confirm whether to remove this paragraph entirely.
...
public protocol LambdaResponseWriter: ~Copyable {
    /// Write a response part into the stream. The HTTP response is started lazily before the first call to `write()`. Bytes written to the writer are streamed continually.
...
finishing functions from being called multiple times. // TODO: Do we want to mention that we will automatically call
`finish()` for the user if they have not called it?
...
public protocol StreamingLambdaHandler: ~Copyable {
    ...
    ///                     report an error to the invoker. // TODO: Change this as reportError(_:) has been removed.
...
returned. This is because the `handle(...)` function does not have a return type and the result is reported through the
underlying `LambdaResponseWriter`. After calling the `LambdaResponseWriter.finish()` function, the `handle(...)`
function is free to implement any background work.
