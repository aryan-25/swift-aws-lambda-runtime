...
1. `LambdaHandlerAdapter` wraps `LambdaHandler` with `LambdaWithBackgroundProcessingHandler`. This is achieved by taking
   ...
   function of the `LambdaResponseWriter`.

2. `LambdaCodableAdapter` wraps `LambdaWithBackgroundProcessingHandler` with `StreamingLambdaHandler`. This is achieved
   by wrapping the `LambdaResponseWriter` with the `LambdaResponseStreamWriter` provided by `StreamingLambdaHandler`. A
   call to the `write(_:)` function of `LambdaResponseWriter` is translated into a call to the `writeAndFinish(_:)`
   function of `LambdaResponseStreamWriter`.
...
`LambdaResponseStreamWriter.finish()`. For handlers conforming to the `LambdaHandler` protocol, this solution was
considered:
...
1. `LambdaHandler`: Most common use-case. JSON-in, JSON-out. Does not support background work execution. An intuitive
   ...
   to the concept of sending their response through a `LambdaResponseWriter`. `LambdaHandler` can be very cleanly
...
3. `StreamingLambdaHandler`: This is the base handler protocol which is intended to be used directly only for advanced
   ...
   computed result (as `ByteBuffer`) can either be streamed or sent all at once. After closing the
   `LambdaResponseStreamWriter` (with `finish()` or `writeAndFinish()`), any background work can be implemented.
