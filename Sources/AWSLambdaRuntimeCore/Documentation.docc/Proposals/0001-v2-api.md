...
- v1.1:
  ...
  - Remove the `addBackgroundTask(_:)` method from `LambdaContext` and introduce the
    `LambdaWithBackgroundProcessingHandler` protocol as a solution
...
The proposed design of passing a `LambdaResponseWriter` as an argument to the `handle(...)` function of objects
conforming to `StreamingLambdaHandler` already allows for background tasks to be run _after_ the response has been
reported to the AWS control plane. This is because the `handle(...)` function does not have a return type and the result
is reported through the underlying `LambdaResponseWriter`. After calling the `LambdaResponseWriter.finish()` function,
the `handle(...)` function is free to implement any background work.

Note that the `handle(...)` function of the `LambdaHandler` protocol _does_ however return a generic `Output` object
instead of writing the response to a `LambdaResponseWriter`. The reason for this is that `LambdaHandler` is designed to
be used alongside `LambdaCodableAdapter` for the common JSON-in, JSON-out use-case, where the response cannot be
streamed (since each partial output has to be valid JSON for the output encoding that `LambdaCodableAdapter` performs).

For use-cases requiring JSON abstraction _as well_ as the ability to run background tasks after returning the response,
a new handler protocol, `LambdaWithBackgroundProcessingHandler`, should be used. This protocol differs from
`LambdaHandler` in two ways:

1. The `handle(...)` function has an additional argument accepting an object conforming to a new protocol
   `ResponseSender`. The `ResponseSender` protocol is non-copyable and specifies a consuming `send(_:)` function
   (instead of the `write` and `finish` semantics of `LambdaResponseWriter` since the response is not being streamed).
2. The `handle(...)` function does not explicitly return and is therefore able to run background tasks after calling the
   `send(_:)` function of `ResponseSender`.

```swift
public protocol ResponseSender<Output>: ~Copyable {
    associatedtype Output

    /// Sends the generic Output object (representing the computed result of the handler)
    /// to the AWS Lambda response endpoint.
    /// This function is not async since there is no underlying streaming and does not throw either.
    /// This function simply serves as a mechanism to return the computed result from a handler function
    /// without an explicit `return`.
    consuming func send(_: Output)
}
...
public protocol LambdaWithBackgroundProcessingHandler {
    ...
    associatedtype Event
    /// Generic output type
    /// This is the type that the handle() function will send through the ``ResponseSender``.
    associatedtype Output
    ...
    func handle(
        _ event: Event,
        responseSender: consuming some ResponseSender<Output>,
        context: LambdaContext
    ) async throws
}
```
...
struct BackgroundProcessingHandler: LambdaWithBackgroundProcessingHandler {
    ...
    func handle(
        _ event: Input,
        responseSender: consuming some ResponseSender<Greeting>,
        context: LambdaContext
    ) async throws {
        // Report the computed result
        responseSender.send(Greeting(echoedMessage: event.message))
...
For handlers conforming to the `StreamingLambdaHandler`, `addBackgroundTask(_:)` was unnecessary as background work
could be executed within the `handle(...)` function in a structured concurrency manner after the call to
...
Yet this poses difficulties when the user wishes to use the context in the `handle(...)` function in the background
work.
